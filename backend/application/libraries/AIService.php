...
class AiService
{
    ...
    private function callOpenRouter($prompt)
    {
        ...
        if (!$response['error']) {
            ...
            if (empty($data['choices'][0]['message']['content'])) {
                $rawMsg = $data['error']['message'] ?? (is_string($response['response']) ? $response['response'] : json_encode($response['response']));
                log_message('error', 'Invalid OpenRouter response: ' . $rawMsg);
                return ['error' => true, 'msg' => 'Invalid OpenRouter response: ' . $rawMsg];
            }

            $content = trim($data['choices'][0]['message']['content']);

            // Resilient JSON extraction in case model returns ```json ... ``` codeblocks
            ...
            // If direct questions array is found
            if (!empty($contentdata['questions']) && is_array($contentdata['questions'])) {
                log_message('error', 'OpenRouter questions generated: ' . count($contentdata['questions']));
                return $contentdata['questions'];
            }

            // Fallback if returned directly as an array of questions
            if (is_array($contentdata) && isset($contentdata[0]['question'])) {
                return $contentdata;
            }

            log_message('error', 'OpenRouter questions json format error: ' . $content);
            return ['error' => true, 'msg' => 'OpenRouter questions JSON structure could not be parsed: ' . substr($content, 0, 200)];
        } else {
            $result = [
                'error'     => true,
                'msg'       => $response['msg'],
