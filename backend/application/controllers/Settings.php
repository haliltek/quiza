...
class Settings extends CI_Controller
{
    ...
    public function system_utilities()
    {
        try {
            if (!$this->session->userdata('isLoggedIn')) {
                ...
                if ($this->input->post('btnadd')) {
                    if (!has_permissions('update', 'system_utilities')) {
                        $this->session->set_flashdata('error', lang(PERMISSION_ERROR_MSG));
                    } else {
        ...
        try {
            if (!$this->session->userdata('isLoggedIn')) {
                ...
                if (!$this->session->userdata('authStatus')) {
                    ...
                    if ($this->input->post('btnadd')) {
                        if (!has_permissions('update', 'contact_us')) {
                            $this->session->set_flashdata('error', lang(PERMISSION_ERROR_MSG));
                        } else {
        ...
        try {
            if (!$this->session->userdata('isLoggedIn')) {
                ...
                if (!$this->session->userdata('authStatus')) {
                    ...
                    if ($this->input->post('btnadd')) {
                        if (!has_permissions('update', 'term_conditions')) {
                            $this->session->set_flashdata('error', lang(PERMISSION_ERROR_MSG));
                        } else {
        ...
        try {
            if (!$this->session->userdata('isLoggedIn')) {
                ...
                if (!$this->session->userdata('authStatus')) {
                    ...
                    if ($this->input->post('btnadd')) {
                        if (!has_permissions('update', 'privacy_policy')) {
                            $this->session->set_flashdata('error', lang(PERMISSION_ERROR_MSG));
                        } else {
        ...
        try {
            if (!$this->session->userdata('isLoggedIn')) {
                ...
                if (!$this->session->userdata('authStatus')) {
                    ...
                    if ($this->input->post('btnadd')) {
                        if (!has_permissions('update', 'how_to_play')) {
                            $this->session->set_flashdata('error', lang(PERMISSION_ERROR_MSG));
                        } else {
        ...
        try {
            if (!$this->session->userdata('isLoggedIn')) {
                ...
                if (!$this->session->userdata('authStatus')) {
                    ...
                    if ($this->input->post('btnadd')) {
                        if (!has_permissions('update', 'about_us')) {
                            $this->session->set_flashdata('error', lang(PERMISSION_ERROR_MSG));
                        } else {
                ...
                if (!has_permissions('read', 'ai_settings')) {
                    ...
                    if ($this->input->post('btnadd')) {
                        if (!has_permissions('update', 'ai_settings')) {
                            ...
                            if ($ai_provider == 'gemini' || $ai_provider == 'openai' || $ai_provider == 'openrouter') {
                                if ($ai_provider == 'gemini') {
                                    $model = $this->input->post('gemini_model');
                                    $apiKey = $this->input->post('gemini_api_key');
                                    if ($model && $apiKey) {
                                        $checkData = $this->checkGeminiKeyAndModel($apiKey, $model);
                                    }
                                } else if ($ai_provider == 'openai') {
                                    $model = $this->input->post('openai_model');
                                    $apiKey = $this->input->post('openai_api_key');
                                    if ($model && $apiKey) {
    ...
    function checkOpenAIKeyAndModel($apiKey, $model)
    {
        ...
        if ($httpCode === 200) {
            return ['status' => true, 'message' => 'API key and model are valid'];
        }

        if ($httpCode === 401) {
            return ['status' => false, 'message' => 'API key not valid. Please pass a valid API key.'];
        }

        if ($httpCode === 404) {
            return ['status' => false, 'message' => 'Model not found or incorrect model name'];
        }

        return ['status' => false, 'message' => 'Unknown error', 'response' => $response];
    }


    function checkGeminiKeyAndModel($apiKey, $model)
    {
        ...
        switch ($httpCode) {
            ...
            case 403:
                return [
                    'status'  => false,
                    'message' => 'Invalid API key, billing issue, or Generative AI API not enabled'
                ];

            case 404:
                return [
                    'status'  => false,
                    'message' => 'Model not found or incorrect model name'
                ];
    ...
    function checkOpenRouterKeyAndModel($apiKey, $model)
    {
        ...
        curl_setopt_array($ch, [
            ...
            CURLOPT_HTTPHEADER => [
                ...
                "X-Title: " . (is_settings('app_name') ? is_settings('app_name') : "Quiza")
            ],
        ...
        $response = curl_exec($ch);
        log_message('error', 'OpenRouter probe response: ' . $response);

        $httpCode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
        curl_close($ch);

        if ($httpCode === 200) {
            return ['status' => true, 'message' => 'OpenRouter API anahtarı ve model başarıyla doğrulandı'];
        }
        ...
        if ($httpCode === 402 || stripos($errorMsg, 'credits') !== false || stripos($errorMsg, 'payment') !== false) {
            return ['status' => false, 'message' => 'OpenRouter hesabınızda yetersiz bakiye: ' . $errorMsg];
        }

        if (!empty($errorMsg)) {
            return ['status' => false, 'message' => 'OpenRouter Hatası (' . $httpCode . '): ' . $errorMsg];
        }

        return ['status' => false, 'message' => 'OpenRouter bağlantı hatası (HTTP ' . $httpCode . ')', 'response' => $response];
    }
}
