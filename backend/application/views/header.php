    ...
    <ul class="navbar-nav navbar-right">
        <li class="dropdown dropdown-list-toggle">
            <a href="#" data-toggle="dropdown" class="nav-link nav-link-lg" aria-expanded="false">
                <h6>
                    ...
                    <?= ucwords(get_user_permissions($this->session->userdata('authId'))['language'] ?? 'English') ?>
                </h6>
            </a>
            <div class="dropdown-menu dropdown-menu-right">
                <?php foreach (panel_languages() as $value) { ?>
                    <a href="<?php echo base_url('System_Languages/switch/') . $value; ?>" class="dropdown-item text-decoration-none <?php if (($permissions = get_user_permissions($this->session->userdata('authId'))) && isset($permissions['language']) && $permissions['language'] == $value) echo 'active'; ?>">
        ...
        <li class="dropdown">
            <a href="<?= base_url(); ?>" data-toggle="dropdown" class="nav-link dropdown-toggle  nav-link-lg nav-link-user">
                ...
                <div class="d-sm-none d-lg-inline-block"><?= lang('hi'); ?>, <?= ucwords($this->session->userdata('authName') ?? 'User'); ?></div>
            </a>
            <div class="dropdown-menu dropdown-menu-right">
...
<div class="main-sidebar">
    <aside id="sidebar-wrapper">
        <ul class="sidebar-menu">
            ...
            <?php } ?>
            <?php if (has_permissions('read', 'backup_data')) { ?>
                <li>
                    <a class="nav-link" href="<?= base_url(); ?>backup"><em class="fas fa-cloud"></em> <span>Backup Data</span></a>
                </li>
            <?php } ?>
        </ul>
    </aside>
