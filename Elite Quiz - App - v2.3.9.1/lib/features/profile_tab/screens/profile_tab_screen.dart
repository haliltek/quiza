...
final class ProfileTabScreenState extends State<ProfileTabScreen>
  ...
  Widget _buildListViewItem(MenuItem item) {
    ...
    } else if (item.name == 'quizLanguage') {
      final supported =
          context.watch<SystemConfigCubit>().supportedQuizLanguages;
      final currId = context.watch<QuizLanguageCubit>().languageId;
      final active = supported.firstWhere(
        (e) => e.id == currId,
    ...
    final itemTitle = item.name == 'quizLanguage'
        ? 'Hedef Sınavım'
        : (context.tr(item.name) ?? item.name);
