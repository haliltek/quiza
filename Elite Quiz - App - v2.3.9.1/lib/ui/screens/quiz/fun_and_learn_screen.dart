...
class _FunAndLearnScreen extends State<FunAndLearnScreen> {
  ...
  late final _ytController = YoutubePlayerController.fromVideoId(
    videoId: _comprehension.contentData,
  );

  @override
  void dispose() {
    super.dispose();
    unawaited(_ytController.close());
  }
  ...
  Widget build(BuildContext context) {
    final player = Theme(
      data: Theme.of(context).copyWith(
        extensions: [
          YoutubePlayerTheme(
            progressBarActiveColor: context.primaryColor,
            progressBarBufferedColor: context.primaryTextColor.withValues(
              alpha: .5,
            ),
            progressBarBackgroundColor: context.surfaceColor.withValues(
              alpha: .5,
            ),
            controlsColor: context.primaryColor,
          ),
        ],
      ),
