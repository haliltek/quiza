...
class _AnimatedSubcategoryContainerState
  ...
  void _onTapSubcategory(Subcategory subcategory) {
    //If no subcategory has no level then play the quiz
    if (!subcategory.hasLevel) {
      if (locked) {
        showUnlockPremiumCategoryDialog(
          context,
          categoryId: widget.category.id!,
      ...
      if (widget.quizType == QuizTypes.quizZone) {
        Navigator.of(context).pushNamed(
          ...
          arguments: {
            ...
            'isPlayed': widget.subcategory.isPlayed,
          },
        );
        return;
      } else if (widget.quizType == QuizTypes.multiMatch) {
        context.pushNamed(
          Routes.multiMatchQuiz,
          arguments: MultiMatchQuizArgs(
            categoryId: widget.category.id!,
  ...
  int getWithoutLevelQuestionsCount(Subcategory subcategory) {
    if (widget.quizType == QuizTypes.multiMatch) {
      final multiMatchQuestionsCount = context
          .read<SystemConfigCubit>()
          .systemConfigModel!
          .multiMatchTotalQuestion;
      final subcategoryQuestionsCount = int.parse(subcategory.noOfQue!);
      return subcategoryQuestionsCount > multiMatchQuestionsCount
          ? multiMatchQuestionsCount
          : subcategoryQuestionsCount;
    } else {
      ///Default quiz zone questions

      final quizZoneTotalQuestion = context
          .read<SystemConfigCubit>()
          .systemConfigModel!
          .quizZoneTotalQuestion;
      final subcategoryQuestionsCount = int.parse(subcategory.noOfQue!);
      return subcategoryQuestionsCount > quizZoneTotalQuestion
          ? quizZoneTotalQuestion
          : subcategoryQuestionsCount;
    }
  }
                      ...
                      children: [
                        ...
                        RichText(
                          text: TextSpan(
                            ...
                            children: [
                              // Show level information only if subcategory has levels
                              if (subcategory.hasLevel) ...[
                                TextSpan(
                              ...
                              const WidgetSpan(child: SizedBox(width: 5)),
                              TextSpan(
                                text: subcategory.hasLevel
                                    ? subcategory.noOfQue
                                    : getWithoutLevelQuestionsCount(
                                        subcategory,
                                      ).toString(),
