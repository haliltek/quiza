...
class _ResultScreenState extends State<ResultScreen> {
  ...
  void onPageBackCalls() {
    if (widget.quizType == QuizTypes.funAndLearn &&
      ...
      // Check if this is a level-based quiz or direct category/subcategory play
      if (widget.level == '0') {
        // Direct category/subcategory play (NO levels)
        // Only refresh on first win to avoid unnecessary API calls
        if (_isWinner && !widget.isPlayed) {
          if (widget.subcategoryId == '' || widget.subcategoryId == null) {
        ...
        // Level-based quiz - always refresh to update unlocked levels
        if (widget.subcategoryId == '' || widget.subcategoryId == null) {
