...
class HomeScreenState extends State<HomeScreen>
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        _isGuest ? 'Misafir Yarışmacı' : 'Yarışmacı',
                        textAlign: TextAlign.start,
                        maxLines: 1,
                        overflow: TextOverflow.ellipsis,
                        style: TextStyle(
                          color: context.primaryTextColor.withValues(alpha: .5),
                          fontSize: 12,
  ...
  Widget _buildActiveExamPill() {
    final supported = context.watch<SystemConfigCubit>().supportedQuizLanguages;
    final activeId = context.watch<QuizLanguageCubit>().languageId;
    final currentExam = supported.firstWhere(
      (e) => e.id == activeId,
      orElse: () => supported.isNotEmpty
          ? supported.first
          : const QuizLanguage(
              id: '52',
    ...
    return Padding(
      ...
      child: InkWell(
        onTap: () async {
          await showQuizLanguageSelectorSheet(context);
          if (mounted) {
            setState(() {
              _currLangId = UiUtils.getCurrentQuizLanguageId(context);
            });
                ...
                if (state is UserDetailsFetchSuccess) {
                  ...
                  if (state.userProfile.profileUrl!.isEmpty ||
                    ...
                    if (!profileComplete) {
                      ...
                      if (context.read<AuthCubit>().isNewUser &&
                          Routes.currentRoute == Routes.selectProfile) {
                        return;
                      }
